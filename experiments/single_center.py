...
def run_single_center(root: Path) -> dict[str, dict[str, float]]:
    ...
    condition = resolve_condition(robustness_config, "ideal")
    ...
    for controller_name, scenario in scenarios.items():
        ...
        optimization["finite_horizon"]["prediction"] = deepcopy(condition["prediction"])
        ...
        system.start_run(
            RunMetadata(
                name="single_center_compute_power_coordination",
                controller_name=controller_name,
                condition_name="ideal",
        ...
        all_metrics[controller_name] = dict(summary.metrics)
        store.close()
    return all_metrics
